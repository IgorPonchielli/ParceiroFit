...
import { ArrowLeft, Video, Save, Lock, Unlock, Loader2, Check, Trash2, FileText } from "lucide-react";
...
// In Next.js 15, we must unwrap params with `use()` if passed as promise, 
// or simply pass the ID string directly if handled in page.tsx
interface Props {
  contentId: string;
}

export default function EditarConteudoClient({ contentId }: Props) {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        const [fetchedPlans, fetchedContent] = await Promise.all([
        ...
        if (fetchedContent) {
          if (fetchedContent.profissional_uid !== userProfile.uid) {
    ...
    loadData();
  }, [userProfile?.uid, authLoading, userProfile?.role, contentId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (content.type === "video") {
        ...
        await contentService.updateVideoContent(contentId, updateData);
      } else {
        // Logica para artigo (futura extensão de imgs etc pode entrar no updateData)
        await contentService.updateArticleContent(contentId, updateData);
      }
      ...
      setError(err.message || "Erro ao atualizar o conteúdo.");
    } finally {
  ...
  const handleDelete = async () => {
    if (!content) return;
    const confirmDelete = window.confirm("Tem certeza que deseja deletar este conteúdo? Esta ação não pode ser desfeita.");
    ...
    try {
      if (content.type === "video") {
        await contentService.deleteVideoContent(contentId);
      } else {
        await contentService.deleteArticleContent(contentId);
      }
      router.push(`/${slug}/dashboard`);
    } catch (err: any) {
      setError(err.message || "Erro ao deletar o conteúdo.");
      setIsDeleting(false);
    }
  };

  if (authLoading || isLoadingContent) return <div className="min-h-screen bg-gray-950 flex items-center justify-center"><Loader2 className="w-8 h-8 text-emerald-400 animate-spin" /></div>;
      ...
      <div className="p-6 max-w-3xl mx-auto w-full">
        ...
        <div className="flex items-center justify-between mb-8 pt-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl md:text-3xl font-bold text-white flex items-center gap-2">
                {content?.type === "video" ? <Video className="w-6 h-6 text-emerald-400" /> : <FileText className="w-6 h-6 text-emerald-400" />}
                Editar {content?.type === "video" ? "Vídeo" : "Artigo"}
              </h2>
              <p className="text-gray-400 mt-1">Atualize as informações do seu conteúdo.</p>
            </div>
          </div>
          ...
          >
             ...
             <span className="hidden md:inline font-bold">Deletar</span>
          </button>
        </div>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-3xl p-6 md:p-8 space-y-6">
            ...
            {success && (
              <div className="p-4 bg-emerald-500/10 border border-emerald-500/50 rounded-xl text-emerald-400 text-sm flex items-center gap-2">
                ...
                Conteúdo atualizado com sucesso! Redirecionando...
              </div>
            )}
            ...
            {!isFree && (
              <div className="mt-6 pt-6 border-t border-gray-800 animate-in fade-in slide-in-from-top-4">
                <label className="block text-gray-400 text-sm font-medium mb-4">
                  Quais planos dão acesso a este conteúdo?
                </label>
